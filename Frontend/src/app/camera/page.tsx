...
const CameraFeedPage = () => {
  ...
  const fireDetectionIntervalRef = useRef<NodeJS.Timeout | null>(null);
  const pathCheckIntervalRef = useRef<NodeJS.Timeout | null>(null);

  // Function to check for path data from the map endpoint
  const checkForPath = async () => {
    try {
      const response = await axios.get('http://localhost:5001/get_occupancy_grid');
      const data = response.data;
      
      // Update path state if path data is available
      if (data.path && Array.isArray(data.path) && data.path.length > 0) {
        setPath(data.path);
        // Set alert status to warning when path exists
        setAlertStatus('alert');
      } else {
        setPath([]);
        // Only set to all-clear if no path exists
        setAlertStatus('all-clear');
      }
      
    } catch (error) {
      console.error('Error checking for path:', error);
      // If we can't check for path, maintain current state
    }
  };
  ...
  const detectFire = async () => {
    ...
    try {
      const response = await axios.get('http://localhost:5001/detect_fire');
      const data = response.data;
      ...
      setError('Failed to detect fire. Please try again later.');
    } finally {
  ...
  const fetchImage = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:5001/raw_frame?t=${timestamp}`, {
        responseType: 'blob'
      });
  ...
  useEffect(() => {
    ...
    // Start fire detection immediately
    detectFire();
    
    // Start path checking immediately
    checkForPath();
    
    // Set up polling interval for image refresh (every 2 seconds)
    intervalRef.current = setInterval(fetchImage, 2000);
    
    // Set up fire detection interval (every 15 seconds)
    fireDetectionIntervalRef.current = setInterval(detectFire, 15000);
    
    // Set up path checking interval (every 5 seconds)
    pathCheckIntervalRef.current = setInterval(checkForPath, 5000);
    
    // Clean up function to clear intervals and revoke object URL
    return () => {
      if (intervalRef.current) {
        clearInterval(intervalRef.current);
      }
      if (fireDetectionIntervalRef.current) {
        clearInterval(fireDetectionIntervalRef.current);
      }
  ...
  const handleFireDetectionClick = () => {
    detectFire();
  };

  const handlePathCheckClick = () => {
    checkForPath();
  };
  ...
  return (
    <div>
      ...
      <div className={styles.container}>
        {/* Alert Banner - Now based on path existence rather than fire detection */}
        <div
        ...
        >
          {path.length > 0
            ? `⚠️ WARNING: Fire is Detected!`
            : '✅ All Clear: No Fire detected.'}
        </div>
        
        {/* Fire Detection Status */}
        {fireDetected && (
          <div className={styles.fireStatus}>
            <h3>🔥 Fire Detection Active</h3>
            <p>
              Fire Location: {firePosition ? `(${firePosition.x}, ${firePosition.y})` : 'Position unknown'}
            </p>
            <p>Status: Monitoring fire situation</p>
          </div>
        )}
        ...
        {path.length > 0 && (
          <div className={styles.pathStatus}>
            <h3>🚗 Navigation Path Active</h3>
            <p>Path Length: {path.length} steps</p>
            <p>Status: Vehicle navigation system is active</p>
          </div>
        )}
        ...
        <div className={styles.cameraFeed}>
          <div className={styles.feedHeader}>
            <p className={styles.feedTitle}>Live Camera Feed</p>
        ...
        <div className={styles.controlPanel}>
          ...
          <button 
            ...
            onClick={handleFireDetectionClick}
            disabled={detectionInProgress}
          >
            {detectionInProgress ? 'Detecting Fire...' : 'Manual Fire Detection'}
          </button>
          <button 
            className={styles.controlButton} 
            onClick={handlePathCheckClick}
          >
