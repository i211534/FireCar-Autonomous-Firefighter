///Project/frontendfirecar - Copy - Copy/src/app/map/page.tsx
'use client';
...
const MapPage = () => {
  ...
  const fetchOccupancyGrid = useCallback(async (forceUpdate = false, isRefresh = false) => {
    ...
    try {
      ...
      // 1) occupancy grid & path
      setOccupancyGrid(data.occupancy_grid || []);
      setPath(Array.isArray(data.path) ? data.path : []);
  
      // 2) **always** update the goal/fire marker
      if (data.goal) {
        setGoalX(data.goal.x);
        setGoalY(data.goal.y);
        setFirePosition({ x: data.goal.x, y: data.goal.y });
      }
  
      // 3) car position stays the same
      if (data.car_position) {
        setCarPosition({ x: data.car_position.x, y: data.car_position.y });
      }
  
      // 4) status, lastUpdate, clear errors ...
      if (data.status) setStatus(`Navigation status: ${data.status}`);
