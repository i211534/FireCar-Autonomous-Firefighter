...
def preprocess_grid_for_car(occupancy_grid):
    ...
    edge_width = EDGE_MARGIN
    expanded_grid[0:edge_width, :] = 1  # Top edge
    expanded_grid[height-edge_width:height, :] = 1  # Bottom edge
    expanded_grid[:, 0:edge_width] = 1  # Left edge
    expanded_grid[:, width-edge_width:width] = 1  # Right edge
...
def a_star_car_pathfinding(occupancy_grid, start, goal, heuristic_type='euclidean', allow_diagonal=False):
    ...
    distance_map = cv2.distanceTransform((car_grid == 0).astype(np.uint8), cv2.DIST_L2, 3)
        ...
        for i, direction in enumerate(directions):
            ...
            if 0 <= neighbor[0] < height and 0 <= neighbor[1] < width:
                ...
                if car_grid[neighbor] == 0:
                    # Start with the basic move cost
                    tentative_g_score = g_score[current] + move_costs[i]
                    
                    # Add a turning penalty if the motion direction changes
                    ...
                    # Add clearance penalty: if the neighbor is closer to an obstacle, add extra cost
                    clearance_penalty = 0
                    if distance_map[neighbor] < distance_map[current]:
                        lambda_penalty = 1.0  # Tune this factor as needed
                        clearance_penalty = lambda_penalty * (distance_map[current] - distance_map[neighbor])
                    
                    tentative_g_score += clearance_penalty

                    # Update the scores and open set if we have a better path
