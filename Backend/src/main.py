...
def process_frame(frame, last_command, ser, current_direction='NORTH', 
                  ...
                  initial_car_position=None, fire_bboxes=None):
    ...
    segmented_frame, occupancy_grid, car_position = segment_objects_and_create_grid(
        ...
        fire_detections=fire_bboxes  # Pass the fire bounding boxes
    )
    ...
    # If we donï¿½t yet have a goal (e.g. no fire detected), skip pathfinding
    if goal_position is None:
        print("No goal set yet; skipping path planning")
        annotated_frame = save_frame_with_command(segmented_frame, last_command, frame_number)
        return last_command, annotated_frame, occupancy_grid, None, current_direction, is_first_command, previous_positions, goal_position, initial_car_position

    # Always recalculate path in every frame now that we have a goal
    ...
    # Now send the (possibly updated) command:
    
    if command == 's' and near_goal:
        # MODIFIED: Before spraying, turn to face the fire
        if goal_position:
            # Calculate direction towards fire
            fire_direction = get_target_direction(
                goal_position[0] - car_position[0], 
                goal_position[1] - car_position[1]
            )
            
            # If not already facing the fire, turn to face it
            if current_direction != fire_direction:
                print(f"Turning to face fire: Current direction {current_direction}, Target direction {fire_direction}")
                turn_cmd = get_turn_command(current_direction, fire_direction)
                
                # Execute the turn command
                send_command(ser, turn_cmd)
                # Update direction after turning
                current_direction = update_direction(current_direction, turn_cmd)
                print(f"Now facing {current_direction}")
                
                # Add a small delay after turning
                time.sleep(1.0)
                
        print("Stopping near goal - sending 'a' command.")
        send_command(ser, 'a')
        try:
            # log the fire event
            resp = requests.post(
                "http://localhost:5001/firelogs",
                json={
                    "location": f"grid ({goal_position[0]}, {goal_position[1]})",
                    "severity": "high",               # or derive from your detection confidence
                    "suppression_action": "water spray",# match whatever action you used
                    "resolved": True
                },
                timeout=2
            )
            if resp.status_code != 201:
                print(f"Warning: fire-log POST returned {resp.status_code}: {resp.text}")
        except Exception as e:
            print(f"Error logging fire event: {e}")
        annotated_frame = save_frame_with_command(segmented_frame, 'a', frame_number)
        last_command    = 'a'
        goal_position = None
        current_path  = None
        near_goal     = False
        last_command  = 's'
        # return early so nothing else in this frame can overwrite it
        return last_command, annotated_frame, occupancy_grid, current_path, current_direction, False, previous_positions, goal_position, initial_car_position
    # NEW: If the command is 's' (stop) then check the front and back occupancy
    if command == 's' and not near_goal:
    ...
    # Only update direction based on command if we're not near the goal (since we handle that separately above)
    if not (command == 's' and near_goal):
        new_direction = update_direction(current_direction, command)
        send_command(ser, command)
    else:
        new_direction = current_direction  # Keep the direction we set when facing the fire
...
def determine_command_with_directions(current_position, path, current_direction, last_command=None):
    ...
    lookahead_idx = min(nearest_idx + 18, len(path) - 1)
...
def main():
    ...
    while True:
        ...
        # Detect fire first
        detected, position, fire_visualization_frame, fire_bbox = detect_fire(frame, FIRE_WEIGHTS_PATH, confidence_threshold=temp_threshold)
        
        # Initialize fire_bboxes as empty list
        # Ensure fire_bboxes is always a list
        fire_bboxes = []

        if detected and position is not None:
            # If fire is detected, add the bounding box to the fire_bboxes list
            if fire_bbox is not None:
                fire_bboxes.append(fire_bbox)
                print(f"Fire detected at {fire_bbox}, will be excluded from box detection")
        ...
        last_command, segmented_frame, occupancy_grid, current_path, current_direction, is_first_command, previous_positions, goal_position, initial_car_position = process_frame(
            ...
            fire_bboxes=fire_bboxes  # Pass fire bounding boxes
        )
...
def run_flask_app():
    # bring in the api moduleï¿½s app & db
    from api import app as flask_app, db as flask_db
